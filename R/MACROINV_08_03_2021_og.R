...
###remove fall 2018
#Datared<-subset(Datared,Season_Year!="F18")
#Datared$Season_Year<-levels(droplevels(Datared$Season_Year))
...
###aggregate the 3 sub-sampling for macroinvertebrates within each pond for each sampling date
names(Datared)
...
SubFactorsTaxa<- Datared[,c(41:89)]
DataAggTaxa=as.data.frame(aggregate(SubFactorsTaxa, by=list(Datared$Month, 
                                                            Datared$Year, 
                                                            Datared$Pond,
                                                            Datared$Distance_lake,
                                                            Datared$Position,
                                                            ...
                                                            Datared$Season_Year,
                                                            Datared$Season_Year2,
                                                            Datared$Treat), FUN=sum,na.rm = T))
row.names(DataAggEnv)
row.names(DataAggTaxa)

head(DataAggEnv)
DataAggEnv$Pond_dateEnv <-  do.call(paste,c(DataAggEnv[c("Group.3","Group.6")],sep="-"))
DataAggTaxa$Pond_dateTaxa <-  do.call(paste,c(DataAggTaxa[c("Group.3","Group.6")],sep="-"))

DataAggEnv$Pond_dateEnv
DataAggTaxa$Pond_dateTaxa
...
head(DataAggEnv)
names(DataAggEnv)
setnames(DataAggEnv, old = c(1:12), new = c('Month','Year','Pond','Dist_Lake','Position','newdate',
                                            'DaysFromBeg','SeasonUnique',"Season",'Season_Year','Season_Year2','Treat'))

head(DataAggTaxa)
setnames(DataAggTaxa, old = c(1:12), new = c('Month1','Year1','Pond1','Dist_Lake1','Position1','newdate1',
                                             'DaysFromBeg1','SeasonUnique1',"Season1",'Season_Year1','Season_Year21','Treat1'))

head(DataAggEnv)
DataAggTot<- merge(DataAggEnv, DataAggTaxa, by="row.names")
DataAggTot<-DataAggTot[ order(DataAggTot$newdate),]
names(DataAggTot)
head(DataAggTot)
...
DataAggTot<-DataAggTot[ order(DataAggTot$newdate),]
write.csv(DataAggTot,'DataAggTot.csv')
names(DataAggTot)
DataAggTot <- subset(DataAggTot, select = -c(1,33:44,94))
colnames(DataAggTot)[which(names(DataAggTot) == "Pond_dateEnv")] <- "Pond_date"
#write.csv(DataAggTaxa,'DataAggTaxa.csv')
#write.csv(DataAggEnv,'DataAggEnv.csv')
#write.csv(DataAggTot,'DataAggTot.csv')


# compute N taxa and abundance and eveness

head(DataAggTot)
names(DataAggTot)
TaxaAgg <- DataAggTot[,c(32:80)]
names(TaxaAgg)

Abund<- rowSums(TaxaAgg)
Rich <- specnumber(TaxaAgg)
Sh <- diversity(TaxaAgg)
Eve <- Sh/log(Rich)

DataAggTot$Abund <- Abund
DataAggTot$Rich <- Rich
DataAggTot$Sh <- Sh
DataAggTot$Eve <- Eve
row.names(DataAggTot)<-do.call(paste,c(DataAggTot[c("Pond","newdate")],sep="-"))
...
long_data<-DataAggTot%>%
  pivot_longer(c(32:80), names_to = "variables", values_to = "value")
long_data$variables<-as.factor(long_data$variables)

ggplot(long_data, aes(x = variables, y = value)) + 
  geom_bar(stat = "identity")+
  #ylim(0, 1500)+
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1, size=12))


##Total barplot 1st year
DataAggTot1st<-subset(DataAggTot,Season_Year=="W_17"|Season_Year== "SP_18"|Season_Year== "SU_18")
DataAggTot1st$Season_Year_Month<-do.call(paste,c(DataAggTot1st[c("Season_Year","Month")],sep="-"))
DataAggTot1st<-subset(DataAggTot1st,Season_Year_Month!="SU_18-8")#removed as August not present in 2019
library(tidyr)

names(DataAggTot1st)
str(DataAggTot1st)

long_data<-DataAggTot1st%>%
  pivot_longer(c(32:80), names_to = "variables", values_to = "value")
long_data$variables<-as.factor(long_data$variables)
...
##Total barplot 2nd year
DataAggTot2nd<-subset(DataAggTot,Season_Year=="W_18"|Season_Year== "SP_19"|Season_Year== "SU_19")
DataAggTot2nd$Season_Year_Month<-do.call(paste,c(DataAggTot2nd[c("Season_Year","Month")],sep="-"))
DataAggTot2nd<-subset(DataAggTot2nd,Season_Year_Month!="SP_19-3")#removed as march not present in 2018
library(tidyr)

names(DataAggTot2nd)
str(DataAggTot2nd)

long_data<-DataAggTot2nd%>%
  pivot_longer(c(32:80), names_to = "variables", values_to = "value")
long_data$variables<-as.factor(long_data$variables)
...
#remove Nas also from total dataset (env+macroinvertebrates)

DataAggTot_NA<-DataAggTot[complete.cases(DataAggTot), ]
DataAggTot_NA_red<-subset(DataAggTot_NA,Season_Year!="A_18")
DataAggTot_NA_red$Season_Year_Month<-do.call(paste,c(DataAggTot_NA_red[c("Season_Year","Month")],sep="-"))
DataAggTot_NA_red<-subset(DataAggTot_NA_red,Season_Year_Month!="SP_19-3")
DataAggTot_NA_red<-subset(DataAggTot_NA_red,Season_Year_Month!="SU_18-8")

DataAggTot_NA_red$Season_Year<-as.factor(DataAggTot_NA_red$Season_Year)
DataAggTot_NA_red$Season_Year<-droplevels(DataAggTot_NA_red$Season_Year)
DataAggTot_NA_red$Season_Year_Month<-as.factor(DataAggTot_NA_red$Season_Year_Month)
DataAggTot_NA_red$Season_Year_Month<-droplevels(DataAggTot_NA_red$Season_Year_Month)

####indicate 1st and 2nd year variable
DataAggTot_NA_red$Cat_Y<-ifelse(DataAggTot_NA_red$DaysFromBeg<239,"1st","2nd")

names(DataAggTot_NA_red)
TaxaAgg_NA_red<- DataAggTot_NA_red[,c(32:80)]                           

###remove rows and column = 0 in both the total dataset and the taxa dataset
TaxaAgg_NA_red=TaxaAgg_NA_red[,colSums(TaxaAgg_NA_red)!=0]
DataAggTot_NA_red=DataAggTot_NA_red[rowSums(TaxaAgg_NA_red)!=0,]
TaxaAgg_NA_red=TaxaAgg_NA_red[rowSums(TaxaAgg_NA_red)!=0,]

row.names(TaxaAgg_NA_red)
#DataAggTot_NA=DataAggTot_NA[-c(2, 3, 5:7, 11 ,14),]
names(TaxaAgg_NA_red)
Taxadec=decostand(TaxaAgg_NA_red, method="hell")
Taxa.dca <- decorana(Taxadec)


####fist Axis DCA >3 with hellinger transformed taxa dataset-> still use RDA???
###first analysis

###RDA TAXA and ENVIRONMENTAL variables

names(DataAggTot_NA_red)
EnvAgg_NA_red<-DataAggTot_NA_red[,c(13:28, 30)]
...
corvif(EnvAgg_NA_red)
##remove variables which are correlated with COND and WT
EnvAgg_NA_red2<-EnvAgg_NA_red[, !(colnames(EnvAgg_NA_red) %in% c("K", "pH", "DOC", "Ca"))]
corvif(EnvAgg_NA_red2)

taxa.cap1<- rda(Taxadec ~., EnvAgg_NA_red2,  center=T, scale.=TRUE)
vif.cca(taxa.cap1)

taxa.cap1<- rda(Taxadec ~., EnvAgg_NA_red2,  center=T, scale.=TRUE)
taxa.cap0 <- rda(Taxadec ~ 1, data=EnvAgg_NA_red2, scale.=TRUE,center=TRUE)
#step.env <- ordiR2step(taxa.cap0, taxa.cap1, trace=FALSE)
step.env <- ordistep(taxa.cap0, scope=formula(taxa.cap1),direction = 'forward', permutations = 999, trace=0)
step.env
screeplot(step.env)
...
varp<-varpart (Taxadec, ~Cond+NO3+SO4+FL+PO4, ~WT, ~SubVeg,  data = DataAggTot_NA_red)
plot (varp, digits = 1, Xnames = c('Abiotic',"Temperature", "Biotic"), bg = c('navy', 'tomato', "yellow"))
...
with(step.env, points(step.env, display = "species", select=c(3, 6, 12:16, 18, 19 ,26:28, 30,31, 32),scaling=1, pch=21, bg="blue"))
...
names(DataAggTot_NA_red)
EnvAgg_NA_red_pond<-DataAggTot_NA_red[,c(3, 7, 9, 86)]
corvif(EnvAgg_NA_red_pond)
